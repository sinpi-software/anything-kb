...
- **Every tunable and env var name lives in `config.py`.** No bare `os.environ["..."]` or magic numbers in other modules.
...
def test_schedules_come_from_config():
    ...
    schedule set only in the Prefect UI is overwritten on the next deploy."""
    assert serve.POLL_CRON and serve.INGEST_CRON and serve.JOBS_CRON and serve.DRAFT_CRON
```
