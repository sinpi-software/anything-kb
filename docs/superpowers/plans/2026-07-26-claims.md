        ...
        for document in pending:
            ...
            if document.full_text is None:
                try:
                    ...
                    session.commit()
                except fetch.NoReadableTextError as exc:
                    # Deterministic: the third try yields nothing too. Dead-letter by
                    # exhausting the attempts, which is what drops the row out of the
                    # sweep predicate — there is no separate "abandoned" flag to sync.
                    document.attempts = config.MAX_EXTRACT_ATTEMPTS
                    document.error = f"no readable text: {exc}"
                    session.commit()
                    failed += 1
