...
def test_rss_adapter_fetches_through_the_guard(monkeypatch):
    ...
    assert title == "Example News"
    # 3, not 2: Task 3's fix round added a third FEED entry (embedded markup in
    # <link>, clean <guid>) to cover _entry_url's guid-fallback branch.
    assert len(items) == 3
```
...
Expected: 24 passed (16 from Task 3 as amended in its fix round, plus 8 here). Report the
real number observed — never edit a test to match a count written in this plan.
