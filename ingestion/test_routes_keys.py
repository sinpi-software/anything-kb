...
def _own_kb_id(client: TestClient, email: str) -> str:
    from db import get_postgres_session
...
def test_create_key_is_403_when_email_unverified(client: TestClient) -> None:
    ...
    try:
        client.post("/api/auth/register", json={"email": email, "password": "hunter22"})
        kb_id = _own_kb_id(client, email)
        resp = client.post(f"/api/knowledge-bases/{kb_id}/keys", json={"name": "my key"}, headers=LOCALHOST_ORIGIN)
...
def test_create_key_requires_csrf_origin(client: TestClient) -> None:
    ...
    try:
        ...
        resp = client.post(f"/api/knowledge-bases/{kb_id}/keys", json={"name": "my key"})  # no Origin
        assert resp.status_code == 403
    finally:
        _purge_user(email)
...
def test_create_key_is_201_when_verified_and_key_authenticates_against_engine(client: TestClient) -> None:
    ...
    try:
        _register_and_verify(client, email)
        kb_id = _own_kb_id(client, email)
        resp = client.post(f"/api/knowledge-bases/{kb_id}/keys", json={"name": "ci key"}, headers=LOCALHOST_ORIGIN)
        ...
        assert resolve_knowledge_base(raw_key) == kb_id
    finally:
        _purge_user(email)
...
def test_list_keys_never_leaks_raw_key_or_hash(client: TestClient) -> None:
    ...
    try:
        _register_and_verify(client, email)
        kb_id = _own_kb_id(client, email)
        create = client.post(f"/api/knowledge-bases/{kb_id}/keys", json={"name": "listable"}, headers=LOCALHOST_ORIGIN)
        raw_key = create.json()["key"]
        key_hash_prefix = create.json()["id"]

        listing = client.get(f"/api/knowledge-bases/{kb_id}/keys")
...
def test_revoke_key_stops_it_from_resolving(client: TestClient) -> None:
    ...
    try:
        _register_and_verify(client, email)
        kb_id = _own_kb_id(client, email)
        create = client.post(f"/api/knowledge-bases/{kb_id}/keys", json={"name": "revoke-me"}, headers=LOCALHOST_ORIGIN)
        key_id = create.json()["id"]
        raw_key = create.json()["key"]
        assert resolve_knowledge_base(raw_key)  # works before revocation

        revoke = client.delete(f"/api/knowledge-bases/{kb_id}/keys/{key_id}", headers=LOCALHOST_ORIGIN)
        ...
        listing = client.get(f"/api/knowledge-bases/{kb_id}/keys").json()
...
def test_revoke_unknown_key_is_404(client: TestClient) -> None:
    ...
    try:
        _register_and_verify(client, email)
        kb_id = _own_kb_id(client, email)
        resp = client.delete(f"/api/knowledge-bases/{kb_id}/keys/{uuid.uuid4()}", headers=LOCALHOST_ORIGIN)
...
def test_keys_require_login(client: TestClient) -> None:
    ...
    assert client.get(f"/api/knowledge-bases/{kb_id}/keys").status_code == 401
    assert (
        client.post(f"/api/knowledge-bases/{kb_id}/keys", json={"name": "x"}, headers=LOCALHOST_ORIGIN).status_code
        == 401
    )
