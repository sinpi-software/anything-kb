...
router = APIRouter(prefix="/api/knowledge-bases", tags=["API keys"], dependencies=[Depends(require_csrf)])
...
@router.post("/{kb_id}/keys", response_model=ApiKeyCreateResponse, status_code=status.HTTP_201_CREATED)
def create_key(
    kb_id: str,
...
@router.get("/{kb_id}/keys", response_model=list[ApiKeyOut])
def list_keys(kb_id: str, user: User = Depends(current_user)) -> list[ApiKeyOut]:  # noqa: B008
    with get_postgres_session() as session:
        require_membership(session, user.id, kb_id, "admin")
...
@router.delete("/{kb_id}/keys/{key_id}", status_code=status.HTTP_204_NO_CONTENT)
def revoke_key(kb_id: str, key_id: str, user: User = Depends(current_user)) -> None:  # noqa: B008
    with get_postgres_session() as session:
        require_membership(session, user.id, kb_id, "admin")
