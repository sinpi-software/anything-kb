...
**Files:**
- Create: `claims/pyproject.toml`, `claims/config.py`, `claims/db.py`, `claims/models.py`, `claims/alembic.ini`, `claims/alembic/env.py`, `claims/alembic/script.py.mako`, `claims/.python-version`
- Modify: `.env.sample`
...
- [ ] **Step 10: Write `claims/test_models.py`**
...
Create `claims/test_submit.py`. Reuse the header block from `test_models.py` verbatim — the `import os`, `import config`, the `os.environ[...]` assignment, and `_require_test_postgres()` — then:
...
Append to `claims/test_extract.py`. Put the `test_models.py` header block (the `os.environ[...]` assignment and `_require_test_postgres()`) at the **top** of the file, above the existing gate tests, then append:
...
Prepend the `test_models.py` header block to `claims/test_verify.py` (the `import os`, `import config`, the `os.environ[...]` assignment, and `_require_test_postgres()`, above all other imports), then append:
...
Create `claims/test_report.py`. Prepend the `test_models.py` header block (through `_require_test_postgres()`), then:
...
- `test_extract.py` and `test_verify.py` are each built in two steps (pure helpers
  first, then Postgres-backed flow tests) because the header block that redirects
  `CLAIMS_POSTGRES_URL` must sit above all other imports. The plan says explicitly to
